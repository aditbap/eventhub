...
import { ArrowLeft, ChevronRight, User as UserIcon, Mail, Lock, Phone, UserRound as GenderIcon, Plus, Loader2, LogOut } from 'lucide-react';
import { useRouter } from 'next/navigation';
import Link from 'next/link'; // For future edit pages
...
export default function SettingsPage() {
  ...
  // AppLayout or AuthContext should handle redirect if !user after loading
  // Adding a fallback here just in case.
  if (!user) { 
    // This might flash briefly if AuthContext hasn't redirected yet.
    return <div className="flex justify-center items-center min-h-screen bg-background"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div>;
  }

  const accountItems: SettingsItemProps[] = [
    { icon: UserIcon, label: 'Full name', value: user.displayName || 'N/A', href: '#' }, // '#' as placeholder
    { icon: Mail, label: 'Email', value: user.email || 'N/A' }, // Email typically not editable or via specific flow
    { icon: Lock, label: 'Change Password', value: '••••••••••••', href: '/new-password' }, // Link to password reset/change flow
    { icon: Phone, label: 'Day of birth', value: '02/02/2005', href: '#' }, // Placeholder data and href
    { icon: GenderIcon, label: 'Gender', value: 'Male', href: '#' }, // Placeholder data and href
  ];
  ...
  return (
    <div className="min-h-screen bg-background pb-20"> {/* pb-20 for bottom nav space */}
      ...
      {/* Avatar and Change Picture Section */}
          ...
          {accountItems.map((item, index) => (
            <React.Fragment key={item.label}>
              <SettingsListItem
                ...
                value={item.value}
                href={item.href}
                // className={index === 0 ? "rounded-t-lg" : index === accountItems.length -1 ? "rounded-b-lg" : ""}
              />
